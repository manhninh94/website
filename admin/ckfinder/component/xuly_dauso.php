...
{
    $sql="delete from dauso where ID='".$_REQUEST['delete']."'";
    mysql_query($sql);
    echo"<script>document.location.href='?task=".$obj."' </script>";
}
if(isset($_POST['ok'])) /* xử lý thêm mới*/
{
    $tennhom=$_POST['txt_dauso'];
    $sql="INSERT INTO dauso VALUES('','$tennhom')";
...
if(isset($_POST["edit"]))/* xử lý  sửa*/
{
    $id=$_POST['txtid'];
    $tennhom=$_POST['txt_dauso'];
    $sql="UPDATE  `dauso` SET
	`DauSo` =  '".$tennhom."' WHERE `ID` ='".$id."'";
    mysql_query($sql);
    //echo $sql;
    echo"<script>document.location.href='?task=".$obj."' </script>";
}
