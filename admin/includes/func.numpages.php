...
function getNumPages($total, $url,$num_record_show='',$page=''){
		...
		?>
			...
			if($page>1)
			{
				 echo '<li class="but_nextprew"><a href="?task='.$url.'&page='.($page-1).'" >Prew</a></li>';
			}
			if($page>=4)
			{
			echo '<li><a href="">...</a></li>';
			...
			if($page>2&& $page+2<=$total_pages)
			{
				...
				for($i;$i <= $n;$i++){ 
				if($i!=$page) { 
					    echo '<li><a href="?task='.$url.'&page='.$i.'" >'.$i.'</a></li>';
					} else { 
						echo '<li class="active" ><a href="?task='.$url.'&page='.$i.'" >'.$i.'</a></li>';
					} 
			...
			elseif($page==$total_pages||$page==$total_pages-1)
			{
				...
				if($i!=$page) { 
						...
						echo '<a href="?task='.$url.'&page='.$i.'" ><li class="active" >'.$i.'</a></li>';
					} 
					}
			}
			else
			...
			if($page<$total_pages)
			{
				 echo '<li  class="but_nextprew"><a href="?task='.$url.'&page='.($page+1).'">Next</a></li>';
			}
			}
			else
			{
